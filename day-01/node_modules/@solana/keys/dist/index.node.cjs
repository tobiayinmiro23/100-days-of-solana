...
function assertGrindRegexIsValid(regex) {
  ...
  for (const character of stripped) {
    if (!isBase58Character(character)) {
      throw new errors.SolanaError(errors.SOLANA_ERROR__KEYS__INVALID_BASE58_IN_GRIND_REGEX, {
        character,
        source: regex.source
      });
    }
...
async function grindKeyPairs(config) {
  ...
  while (found.length < amount) {
    ...
    const batch = await promises.getAbortablePromise(
      Promise.all(
...
async function writeKeyPair(keyPair, path$1, config = {}) {
  ...
  await promises$1.mkdir(path.dirname(path$1), { recursive: true });
  await promises$1.writeFile(path$1, JSON.stringify(Array.from(bytes)), {
